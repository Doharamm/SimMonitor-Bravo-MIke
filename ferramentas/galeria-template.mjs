// Template da folha de contato de exames, compartilhado por quem gera a página.
//
// Usado por:
//   ferramentas/gerar-galeria-exames.mjs  → versões do próprio SimMonitor
//   ferramentas/empacotar-exames.mjs      → pacote portátil para outros projetos
//
// Fica separado para as duas não divergirem com o tempo.

export const esc = s => String(s).replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));

// exams    : [{kind, id, src, label}]
// uso      : Map id → ['caso · etapa', …]   (pode ser vazio)
// leituras : { id: {texto, conf} }          (pode ser vazio)
// base     : '' para caminhos relativos, ou 'https://…/' para absolutos
// aviso    : HTML extra dentro da tarja de aviso (ou '')
export function montarGaleria({ exams, uso = new Map(), leituras = {}, base = '', aviso = '' }) {
  const cartao = e => {
...
Clique na imagem para abrir em tamanho real, ou em <b>copiar id</b> para pegar o identificador.</p>
...
que estiver errado antes de usar em aula.${aviso}</p>
<div class="barra">
